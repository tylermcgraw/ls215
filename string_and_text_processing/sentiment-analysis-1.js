...
function sentiment(text) {
  ...
  let positiveCount = 0;
  let negativeCount = 0;
  textExcerpt.toLowerCase()
    .match(/[a-z']+/g)
    .forEach(word => {
      let positiveWord = positiveWords.find(posWord => word.match(posWord));
      let negativeWord = negativeWords.find(negWord => word.match(negWord));
      if (positiveWord) {
        positiveCount += 1;
        positiveSentiments.push(positiveWord);
      } else if (negativeWord) {
        negativeCount += 1;
        negativeSentiments.push(negativeWord);
      }
    });
